...
module GitHosting
  ...
  def self.mirror_push_public_key
    if @@mirror_pubkey.nil?
      ...
      %x[cat '#{Setting.plugin_openproject_gitolite['gitolite_ssh_private_key']}' | #{GitHosting.git_user_runner} 'cat > ~/.ssh/gitolite_admin_id_rsa ' ]
      %x[cat '#{Setting.plugin_openproject_gitolite['gitolite_ssh_public_key']}' | #{GitHosting.git_user_runner} 'cat > ~/.ssh/gitolite_admin_id_rsa.pub ' ]
      ...
      pubk =    ( %x[cat '#{Setting.plugin_openproject_gitolite['gitolite_ssh_public_key']}' ]  ).chomp.strip
  ...
  def self.get_tmp_dir
    tmp_dir = (Setting.plugin_openproject_gitolite['gitTempDataDir'] || TEMP_DATA_DIR)
  ...
  def self.get_bin_dir
    script_dir = Setting.plugin_openproject_gitolite['gitScriptDir'] || SCRIPT_DIR
    if @@previous_git_script_dir != script_dir
      ...
      if script_dir[0,1] == "/"
        ...
        @@git_hosting_bin_dir = File.join(Gem.loaded_specs['openproject-gitolite'].full_gem_path.to_s, script_dir, SCRIPT_PARENT).to_s+"/"
      end
    end
  ...
  def self.update_git_exec
    ...
    gitolite_key=Setting.plugin_openproject_gitolite['gitolite_ssh_private_key']
